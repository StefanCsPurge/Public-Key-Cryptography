# Team work by: Mihalache Stefan, Neghina Dragos, Custura Octavian
import math
import random
import time
...
def generate_large_random_prime() -> int:
    lower_bound = 43424324
    upper_bound = 2432423432
...
def encode_text(word: str, key: int, p: int = 31) -> (int, int):
    value = 0
    current = 0
...
def decode_text(encoded_value: int, key: int, quotient: int = 0, p: int = 31) -> str:
    res = ""
...
if __name__ == '__main__':
    ...
    c = encrypt(remainder, public_key)  # ciphertext
    print("Encoded message:\t", c)

    # Decryption
    print("\nAlice gets the dank message in the Wonderland and decrypts it:")
